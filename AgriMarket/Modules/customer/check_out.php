...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["product_id"]) && isset($_POST["quantity"])) {
        $source = 'buy_now'; // Set source as "buy now"
        $selected_products = [
            [
                'product_id' => (int) $_POST["product_id"], // Get product ID
    ...
    elseif (isset($_POST["selected_products"])) {
        $source = 'cart'; // Set source as "cart"
        $selected_products = json_decode($_POST["selected_products"], true); // Decode JSON data

        // Validate JSON data
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($selected_products)) {
            die("Invalid cart data."); // Handle invalid JSON
        }
        ...
        die("No products selected."); // Handle case where no products are selected
    }
    ...
    if (!empty($selected_products)) {
        ...
        if (!$stmt) {
            die("Database error: " . $conn->error); // Handle query preparation error
        }

        // Bind parameters dynamically
        $types = str_repeat('i', count($product_ids)); // Define parameter types
        $stmt->bind_param($types, ...$product_ids);

        // Execute query
        if (!$stmt->execute()) {
            die("Query failed: " . $stmt->error); // Handle query execution error
        }

        $result = $stmt->get_result();
        $products_from_db = [];

        // Store products in an associative array with product_id as key
        ...
        foreach ($selected_products as &$product) {
            ...
            if (isset($products_from_db[$product_id])) {
                $product['name'] = $products_from_db[$product_id]['product_name']; // Add product name
                $product['price'] = $products_from_db[$product_id]['unit_price']; // Add unit price
                $product['image'] = $products_from_db[$product_id]['product_image']; // Add product image
                $product['subtotal'] = $product['price'] * $product['quantity']; // Calculate subtotal

                $total_amount += $product['subtotal']; // Update total amount
                $total_items += $product['quantity']; // Update total items
            } else {
                // Handle case where product doesn't exist in the database
...
if ($result->num_rows > 0) {
    ...
    $user_address = htmlspecialchars($user_data['home_address']); // Sanitize the address
}
$stmt->close();
?>
                                    ...
                                    <?php if (!empty($selected_products)): ?>
                                        <?php foreach ($selected_products as &$product): ?>
                                            <tr>
                                                ...
                                                <td class="text-center">RM<?php echo number_format($product['price'], 2); ?>
                                                </td>
                                                ...
                                                <td class="text-end">RM<?php echo number_format($product['subtotal'], 2); ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                        ...
                        <div class="accordion" id="paymentAccordion">
                            ...
                            <div class="accordion-item border mb-3">
                                <h2 class="accordion-header">
                                    <div class="form-check p-0">
                                        <input class="form-check-input position-static ms-3 mt-3" type="radio"
                                            name="paymentMethod" id="creditCard" value="credit_card"
                                            data-bs-toggle="collapse" data-bs-target="#creditCardForm"
                                            aria-expanded="true">
                                ...
                                <div id="creditCardForm" class="accordion-collapse collapse"
                            ...
                            <div class="accordion-item border mb-3">
                                <h2 class="accordion-header">
                                    <div class="form-check p-0">
                                        <input class="form-check-input position-static ms-3 mt-3" type="radio"
                                            name="paymentMethod" id="bankTransfer" value="bank_transfer"
                                            data-bs-toggle="collapse" data-bs-target="#bankTransferForm"
                                            aria-expanded="false">
                                ...
                                <div id="bankTransferForm" class="accordion-collapse collapse"
                                    data-bs-parent="#paymentAccordion">
                                    <div class="accordion-body">
                                        <div class="row g-3">
                                            <div class="col-12">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <select class="form-select" id="bankName">
                                                    ...
                                                    <option value="Bank Kerjasama Rakyat Malaysia">Bank Kerjasama Rakyat
                                                        Malaysia</option>
                                                    <option value="Bank Muamalat Malaysia">Bank Muamalat Malaysia
                                                    </option>
                                                    <option value="Bank of China Malaysia">Bank of China Malaysia
                                                    </option>
                                                    <option value="Bank Pembangunan Malaysia">Bank Pembangunan Malaysia
                                                    </option>
                                                    <option value="Bank Pertanian Malaysia (Agrobank)">Bank Pertanian
                                                        Malaysia (Agrobank)</option>
                                                    ...
                                                    <option value="Deutsche Bank Malaysia">Deutsche Bank Malaysia
                                                    </option>
                                                    <option value="Export-Import Bank of Malaysia">Export-Import Bank of
                                                        Malaysia</option>
                                                    <option value="Hong Leong Bank">Hong Leong Bank</option>
                                                    <option value="Hong Leong Islamic Bank">Hong Leong Islamic Bank
                                                    </option>
                                                    <option value="HSBC Bank Malaysia">HSBC Bank Malaysia</option>
                                                    <option value="Industrial and Commercial Bank of China Malaysia">
                                                        Industrial and Commercial Bank of China Malaysia</option>
                                                    <option value="J.P. Morgan Chase Bank Malaysia">J.P. Morgan Chase
                                                        Bank Malaysia</option>
                                                    ...
                                                    <option value="Standard Chartered Bank Malaysia">Standard Chartered
                                                        Bank Malaysia</option>
                                                    <option value="The Bank of Tokyo-Mitsubishi UFJ Malaysia">The Bank
                                                        of Tokyo-Mitsubishi UFJ Malaysia</option>
                                                    <option value="United Overseas Bank Malaysia">United Overseas Bank
                                                        Malaysia</option>
                                                </select>
                                            </div>
                                            <div class="col-12">
                            ...
                            <div class="accordion-item border">
                                <h2 class="accordion-header">
                                    <div class="form-check p-0">
                                        <input class="form-check-input position-static ms-3 mt-3" type="radio"
                                            name="paymentMethod" id="cashOnDelivery" value="cash_on_delivery"
                                            data-bs-toggle="collapse" data-bs-target="#cashOnDeliveryInfo"
                                            aria-expanded="false">
                                ...
                                <div id="cashOnDeliveryInfo" class="accordion-collapse collapse"
                                    data-bs-parent="#paymentAccordion">
                                    <div class="accordion-body">
                                        <p class="mb-0">Pay with cash upon delivery. Please ensure someone is available
                                            at the delivery address to receive the order and make payment.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
