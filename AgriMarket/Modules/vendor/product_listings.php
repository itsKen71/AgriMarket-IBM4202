...
// Fetch vendor details using the user ID
...
// Handle case where vendor profile is not found
if (!$vendor) {
    echo "Error: Vendor profile not found.";
    exit();
}

$vendor_id = $vendor['vendor_id']; // Retrieve the vendor ID

// Check if the vendor has low stock alerts enabled
if ($vendor['has_low_stock_alert']) {
    $lowStockProducts = $productClass->getLowStockProducts($vendor['vendor_id']); // Fetch low stock products
    $lowStockProductsJson = json_encode($lowStockProducts); // Convert to JSON for use in JavaScript
}

// Fetch products based on their status
$approvedProducts = $productClass->getProductsByStatus($vendor_id, 'Approved'); // Approved products
$pendingProducts = $productClass->getProductsByStatus($vendor_id, 'Pending'); // Pending products
$rejectedProducts = $productClass->getProductsByStatus($vendor_id, 'Rejected'); // Rejected products

$uploadLimit = $vendor['upload_limit']; // Retrieve the vendor's upload limit
$pendingCount = $productClass->getPendingProductCount($vendor_id); // Count of pending products
?>
...
<body class="product_listings"
    ...
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <img src="../../assets/img/add-circle.png" alt="Add Product" class="add-btn" id="addProductBtn"
                data-bs-toggle="modal" data-bs-target="#addProductModal" <?php if ($pendingCount >= $uploadLimit)
                ...
                <div class="modal-body">
                    <form action="../../includes/edit_products.php" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="mb-3">
                            ...
                            <!-- Image Preview -->
                            <div class="mt-2 mb-3">
                                <img id="imagePreview" src="" alt="Current Image" class="img-thumbnail"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="stock_quantity" id="editStock" min="0"
                                required>
                        </div>

                        <div class="mb-3">
                            <label for="editWeight" class="form-label">Weight (kg)</label>
                            <input type="number" step="0.01" class="form-control" name="weight" id="editWeight" min="0"
                                required>
                        </div>

                        <div class="mb-3">
                            <label for="editPrice" class="form-label">Price</label>
                            <input type="number" step="0.01" class="form-control" name="unit_price" id="editPrice"
                                min="0" required>
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-3">
                        <form action="../../includes/delete_product.php" method="POST" id="deleteProductForm"
                            class="m-0">
                ...
                <div class="modal-body">
                    <form action="../../includes/add_products.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="weight" name="weight" min="0.01"
                                required>
                        </div>

                        <div class="mb-3">
                            <label for="unitPrice" class="form-label">Price ($)</label>
                            <input type="number" step="0.01" class="form-control" id="unitPrice" name="unit_price"
                                min="0.01" required>
                        </div>
    ...
    <div class="modal fade" id="successSubscribeModal" tabindex="-1" aria-labelledby="successModalLabel"
        aria-hidden="true">\
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
